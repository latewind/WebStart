...
(function($){	 
	 $.fn.lwtables=function(options){
		 var settings = $.extend({
			 open: 1 ,
			 thType:"char,int,double,date,boolean",
			 editable:true,
			 onTest:function(){},
			 aJax:function(){},
			 exother:14
		 	},options);//接受函数参数，将结果赋给settings
...
///////////////////////////////////////////////Method↓/////////////////////////////////////////////
			 function exItem(){//扩展
					$.each(rows,function(index,row){//遍历数组
					 	//row.ex=setSortKey($(row).find('td').eq(col),col);//给每一行增添一个比较值
					 	var $extd=$(row).find('td').eq(settings.exother);
					 
					 	var exdiv=$("<div style='display:none;width:100px;position:relative;'>"+row.exstr+"</div>");
					 	var extr=$("<tr class='exarea' style='display:none;background-color:white;'><td colspan='16'><label>备注：</label><textarea cols=100 rows=4>"+row.exstr+"</textarea></td></tr>");
					 
					 	//$extd.parent().after(exdiv);
					 	$(row).after(extr);//添加扩展
//					 	$(row).before(exdiv);
						$(extr).find('textarea').blur(function(){
							var newtext=$(this).val();
							
							if(newtext!=row.exstr){
								console.log(newtext+"new textarea");
								row.exstr=newtext;
								//$(row).find("td").eq(settings.exother).text(row.exstr);
								//转换为json数组
								updateTr(row);
							}//判断前后两个数是否相等，是否改变
						});
					 	...
					 	var up=true;
//					 	var method=$(row).before;
//					 	method.apply($tbody,exdiv);
					 	$extd.html("<img src='img/down.png' width='25px' height='15px' >").find('img').on("click",function(){
					 		up=!up;
					 		extr.toggle();
					 		if(up){$(this).attr("src","img/down.png")}else{$(this).attr("src","img/up.png");}				 	
					 		console.log("exstr="+row.exstr)
					 	});
						
					 	});			 
				 
			 }
			 function updateTr(row){
				 var sendData={};
					$cells=$(row).find('td');
					$cells.each(function(i){								
						sendData[i]=$(this).html();										
					});
					sendData[settings.exother]=row.exstr;					
					alert(JSON.stringify(sendData));
					
					//插入 ajax（）
					 settings.aJax(sendData);
				 ...
				 $("td").each(function(index){//遍历每一个格子
					 ...
					 if(canedit){
					   $(this).dblclick(function(){
						   ...
						   var thistd=$(this);
						   //index%thnum==0
					        if(!$(this).is('.input')&&index%thnum!=0){
					...
					if(canedit){
					 ...
					 console.log("td width="+tdwidth+"td height"+tdheight);
					 var str=$td.text();//预先存储原来的数据
					 var trbef=$td.parent().text();		 
					 var input=$("<input type='text' id='temp'  style='width:"+tdwidth+"px;height:"+tdheight+"px;margin-top:-1px;margin-bottom:-1px;border:none;' >");
					 console.log("change");
					 input.val(str);//输入框赋值
					 $td.text("");//表格清空
			...
			//列表选择框初始化及搜索框初始化，事件设置
			 ...
			 function selectAndSearch(){
				 ...
				 var searchInput=$("<label>&nbsp&nbsp&nbsp&nbsp搜索:<input id='searchInput' type='text'></label>");
			...
			//分页栏初始化及事件
			 ...
			 function viewTb(data){//输出数据
				...
				// alert("currentP"+currentP+"startTr"+startTr+"endTr"+endTr+"selectNum"+selectNum);//
				 ...
				 exItem()//可优化
				 dbClickOnTd();//可优化，减少范围，yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy 用depath（）可以优化，并能保存事件
				/////////控制分页显示/////////	
...
////////////////////////////////////快速设置page////////////////////////////////////////////			 
			 function changePageQuick(data){//布置分页栏--快捷
					...
					var currentPage=Math.floor(startTr/selectNum)+1;
					console.log(pageNum+" ..."+currentPage+"..."+startTr%selectNum);
					//起始行偏移 解决下拉列表改变每页显示错误
					var moveStart=startTr%selectNum;
					startTr=startTr-moveStart;
